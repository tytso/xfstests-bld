...
func (bisector *GitBisector) packResults(gce *gcp.Service) {
	...
	prefix := fmt.Sprintf("%s/results.%s-%s", bisector.bucketSubdir, server.LTMUserName, bisector.testID)
	...
	check.NoError(err, bisector.log, "Failed to upload results tarball")
